...
export default function AdminPage() {
  ...
  return (
    <div className="flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex flex-col gap-6">
      ...
      {/* Header */}
      <div>
        <span className="text-[10px] uppercase font-black tracking-wider text-blue-500">MUNICIPAL PORTAL</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-start">
        ...
        <div className="lg:col-span-3 glass-panel p-5 bg-opacity-65 dark:bg-opacity-25 border border-card-border overflow-hidden">
          ...
          {currentTab === 'StreetLight' && (
            <div className="mb-6 p-4.5 bg-blue-500/10 border border-blue-500/20 rounded-xl flex gap-3.5 items-start animate-in fade-in slide-in-from-top-2 duration-300">
              ...
              <div className="space-y-1">
                <h4 className="font-extrabold text-sm text-slate-800 dark:text-white">IoT Street Light Hardware Integration</h4>
                <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed font-medium">
                  These complaints are automatically routed here via wireless telemetry hardware installed on municipal lamp posts.
                  Bulb failures, voltage drops, and active circuit cuts trigger alert logs instantly.
                </p>
              </div>
            </div>
          )}
                    ...
                    return (
                      <tr 
                        ...
                        className={`hover:bg-slate-50/50 dark:hover:bg-navy-900/10 transition-colors ${
                          isEditing ? 'bg-blue-500/5 dark:bg-blue-500/5' : ''
                        }`}
                      >
                        {/* ID */}
                        ...
                        <td className="py-4 px-2">
                          {c.imageUrl ? (
                            <div className="relative group h-12 w-16 overflow-hidden rounded-lg border border-slate-200 dark:border-navy-700 bg-slate-100 dark:bg-navy-800 shrink-0 shadow-sm transition-transform duration-300 hover:scale-105">
                        ...
                        <td className="py-4 px-2">
                          <a
                            ...
                            className="inline-flex items-center gap-1 text-[10px] font-bold text-blue-500 hover:text-blue-600 bg-blue-500/5 hover:bg-blue-500/10 border border-blue-500/10 hover:border-blue-500/25 px-2 py-1 rounded-lg transition-all active:scale-95"
                        ...
                        <td className="py-4 px-2">
                          {isEditing ? (
                            ...
                            <div className="space-y-0.5">
                              ...
                              <p className="text-[10px] text-slate-400 dark:text-slate-550">{authority ? authority.role.split(' (')[0] : ''}</p>
                            </div>
                          )}
                        </td>
                        ...
                        <td className="py-4 px-2 text-right">
                          {isEditing ? (
                            ...
                            <button
                              ...
                              className="px-3 py-1.5 bg-blue-600/10 hover:bg-blue-600/20 text-blue-500 dark:text-blue-400 font-bold border border-blue-500/10 rounded inline-flex items-center gap-1 active:scale-95 transition-transform"
                            >
